defmodule StreaksWeb.StreakLive.GridComponent do
  ...
  def handle_event("toggle_day", %{"date" => date}, socket) do
    days = Map.put(socket.assigns.streak.days || %{}, date, next_status(Map.get(socket.assigns.streak.days || %{}, date, "none")))
  ...
  defp build_weeks(year, days) do
    ...
    all_days = Enum.map(Date.range(start_date, end_date), fn date ->
